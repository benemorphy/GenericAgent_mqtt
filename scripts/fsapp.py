...
def _display_text(text):
    ...
    if cleaned:
        return cleaned
    # Fallback: when all visible content was inside stripped tags (e.g. <summary>),
    # extract inner text only from visible tags, skip thinking (shown separately)
    if text:
        # Remove thinking/tool_use completely (tag + content, shown separately in card)
        extracted = re.sub(r'<(?:thinking|tool_use)[^>]*>.*?</(?:thinking|tool_use)[^>]*>', '', text, flags=re.DOTALL)
        # Remove remaining tag markers but keep inner content (summary, file_content, etc.)
        extracted = re.sub(r'</?[a-z_]+>', ' ', extracted)
        extracted = _strip_files(re.sub(r'\n{3,}', '\n\n', extracted)).strip()
        if extracted:
            return extracted
...
def _build_step_detail(resp, tool_calls):
    ...
    注意：整体结果会被截断至 _DETAIL_LIMIT，防止多轮累积超飞书卡片限制。"""
    parts = []
    ...
    if tool_calls:
        ...
        if len(calls_str) > 2000:
            calls_str = calls_str[:2000] + f"\n\n…(已截断,共 {len(calls_str)} 字符)"
...
class _TaskCard:
    ...
    def _step_panel(self, idx, summary, detail, is_latest=False):
        detail = detail or "_(无输出)_"
        if len(detail) > self._DETAIL_LIMIT:
            detail = detail[:self._DETAIL_LIMIT] + f"\n\n…(已截断,共 {len(detail)} 字符)"
        return {
            "tag": "collapsible_panel", "expanded": is_latest,
    ...
    def _build(self):
        ...
        topic = self.final[:500] if self.final else (
            self.steps[-1][0][:500] if self.steps else self.status
        )
        header = f"**{topic}**"
        if self.page_no > 1:
            header += f"\n\n📄 工作卡片 {self.page_no}"
        ...
        # final output 紧接 header 展示（不折叠），避免被面板淹没
        if self.final:
            els.append({"tag": "markdown", "content": self.final[:2000]})
            if len(self.final) > 2000:
                tail = self.final[-200:]
                els.append({"tag": "markdown", "content": f"*…(展开参考底部面板，末段: …{tail})*"})
        for i, (s, d) in enumerate(self.steps, self.turn_base):
            is_latest = (i == self.turn_no) and not self.final
            els.append(self._step_panel(i, s, d, is_latest=is_latest))
        if self.final and len(self.final) > 2000:
    ...
    def hook(ctx):
        try:
            if ctx.get('exit_reason'):
                ...
                # summary可能为空时兜底
                summary = str(ctx.get('rsumm', '')) or '工作中'
                detail = _build_step_detail(ctx.get('response'), ctx.get('tool_calls') or [])
