...
def boards_index(request: Request, user: dict = Depends(require_user)):
    """板块广场"""
    boards = get_boards()
    return render_template("boards/index.html", nav_active="boards", user=user, boards=boards)
...
def boards_search(request: Request, q: str = Query(""), user: dict = Depends(require_user)):
    ...
    return render_template("boards/index.html", nav_active="boards", user=user, boards=get_boards(), search_q=q, search_results=results)
...
def boards_diagnosis(request: Request, user: dict = Depends(require_user)):
    ...
    return render_template("boards/diagnosis.html", nav_active="boards", user=user, posts=posts, total=total)
...
def boards_diagnosis_run(request: Request, user: dict = Depends(require_user)):
    ...
    if not os.path.isfile(scripts):
        return render_template("error.html", nav_active="boards", user=user, error="诊断脚本不存在")
    ...
    try:
        ...
        return render_template("error.html", nav_active="boards", user=user, error=f"诊断触发失败: {e}")
...
def board_posts(request: Request, board_id: str, page: int = Query(1, ge=1),
    ...
    if not board:
        return render_template("error.html", nav_active="boards", user=user, error="板块不存在")
    posts, total = query_posts(board, page=page, q=q)
    limit = 50
    total_pages = max(1, (total + limit - 1) // limit)
    return render_template("boards/board.html", nav_active="boards", user=user, board=board, posts=posts,
