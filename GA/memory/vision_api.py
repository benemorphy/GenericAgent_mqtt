...
DEFAULT_BACKEND = 'claude'               # 默认后端: 'claude' / 'openai' / 'modelscope' / 'local'
# =================================================================
...
# ---- local 后端配置 ----
LOCAL_API_BASE = 'http://localhost:8090'
LOCAL_MODEL = 'qwen3-vl-4b-yoyo-instruct-q8_0.gguf'
LOCAL_MAX_PIXELS = 200000  # 本地模型最大像素数，超过此阈值自动缩小（约450×450）

_DIR = os.path.dirname(os.path.abspath(__file__))
for _p in [os.path.join(_DIR, '..'), os.path.join(_DIR, '../..')]:
    if _p not in sys.path: sys.path.insert(0, _p)

def ask_vision(image_input, prompt="详细描述这张图片的内容", timeout=60, max_pixels=None, backend=DEFAULT_BACKEND):
    if max_pixels is None:
        max_pixels = LOCAL_MAX_PIXELS if backend == 'local' else 1440000
    try:
    ...
    try:
        if backend == 'claude':
            ...
            return _call_openai_compat(
                b64, prompt, timeout,
                apibase=LOCAL_API_BASE, apikey='not-needed', model=LOCAL_MODEL, proxy=None
            )
        else: return f"Error: 未知backend '{backend}'，可选: claude, openai, modelscope, local"
