...
# ── 性能配置（skills: performance_optimization） ──
DIRS_CACHE_TTL = 2      # get_subagent_dirs() 缓存秒数
PIDS_CACHE_TTL = 2      # get_running_pids() 缓存秒数
LOG_TAIL_BYTES = 3000   # 日志文件只读尾部 N 字节
HEAD_BYTES = 4000       # 模型名提取只读文件头 N 字节
MAX_OUTPUT_LEN = 5000   # 单条输出最大显示长度

# ── 错误模式分类（skills: error_handling_patterns） ──
ERROR_PATTERNS = [
    (r'(?i)timeouterror|timed\s*out',      '⏰ 超时错误',   'red'),
    (r'(?i)importerror|modulenotfounderror', '📦 模块缺失',   'orange'),
    (r'(?i)keyerror',                       '🔑 键错误',     'orange'),
    (r'(?i)connectionerror|connectionrefused'
     r'|connectionaborted|connectionreset',  '🔌 连接错误',   'red'),
    (r'(?i)filenotfounderror',              '📁 文件缺失',   'orange'),
    (r'(?i)syntaxerror|indentationerror',    '🔧 语法错误',   'red'),
    (r'(?i)permissionerror',                '🔒 权限错误',   'orange'),
    (r'(?i)memoryerror',                    '💾 内存不足',   'red'),
    (r'(?i)keyboardinterrupt',              '⏹️ 用户中断',   'yellow'),
    (r'(?i)valueerror',                     '📊 值错误',     'orange'),
    (r'(?i)typeerror',                      '📐 类型错误',   'orange'),
    (r'(?i)attributeerror',                 '🧩 属性错误',   'orange'),
    (r'(?i)indexerror|out\s*of\s*range',    '📏 索引越界',   'orange'),
    (r'(?i)httperror|http\s*\d{3}',         '🌐 HTTP错误',   'red'),
    (r'(?i)requests\.exceptions\.',         '📡 请求异常',   'red'),
    (r'(?i)overflowerror|recursionerror',   '🌀 溢出错误',   'red'),
]
...
_dirs_cache = []
_dirs_cache_time = 0.0

def get_subagent_dirs():
    """扫描 temp/ 下所有含 input.txt 的子目录，带 TTL 缓存"""
    global _dirs_cache, _dirs_cache_time
    now = time.time()
    if now - _dirs_cache_time < DIRS_CACHE_TTL:
        return _dirs_cache
    if not TEMP_DIR.exists():
        _dirs_cache = []
        _dirs_cache_time = now
        return []
    dirs = []
...
_pids_cache = {}
_pids_cache_time = 0.0

def get_running_pids():
    """获取所有 Python 进程的 {PID: command_line} 映射，带 TTL 缓存
    优先级: psutil → wmic → tasklist → 空（由启发式兜底）
    """
    global _pids_cache, _pids_cache_time
    now = time.time()
    if now - _pids_cache_time < PIDS_CACHE_TTL:
        return _pids_cache
    pid_map = {}
...
def _classify_errors(log_text: str) -> list:
    """解析 stderr 日志中的已知错误模式，返回 (label, color) 列表"""
    if not log_text:
        return []
    found = []
    for pattern, label, color in ERROR_PATTERNS:
...
def get_subagent_status(task_dir: Path, pid_map: dict):
    ...
    errors = _classify_errors(stderr_log)

    return (status, pid, runtime, latest_output_preview, all_outputs, stdout_log, stderr_log, input_text, enc_label, errors)


def render_agent_card(task_dir: Path, status_info: tuple):
    """渲染单个 subagent 的状态卡片"""
    status, pid, runtime, latest_preview, all_outputs, stdout_log, stderr_log, input_text, enc_label, errors = status_info
    ...
    with st.container():
        ...
        if has_error:
            with cols[6]:
                error_titles = "; ".join([e[0] for e in errors]) if errors else "stderr.log 有内容"
                st.markdown(
                        f'<div style="color:#f44336;font-size:1.2em;" title="{error_titles}">⚠️</div>',
                        unsafe_allow_html=True
                    )
            if errors:
                error_tags = " ".join([f'<span style="background:{c};color:white;padding:2px 8px;border-radius:4px;font-size:0.8em;margin:2px">{l}</span>' for l, c in errors])
                st.markdown(f'<div style="margin-bottom:8px">{error_tags}</div>', unsafe_allow_html=True)
...
<style>
    ...
    .agent-card.error   { border-left: 5px solid #f44336; background: #fff5f5; }
...
""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        refresh_interval = st.slider("刷新间隔(秒)", 1, 10, 3, disabled=not auto_refresh)
        ...
        if launched:
            ...
            elif not re.match(r'^[\w\u4e00-\u9fff-]+$', task_name.strip()):
                st.error("❌ Task Name 只能包含字母、数字、中文、下划线和连字符")
    ...
    with main_placeholder.container():
        ...
        for d in agent_dirs:
            try:
                ...
                status_info = ("error", None, 0, f"读取失败: {e}", [], "", "", "", "", [("读取异常", "red")])
        ...
        for name, status_info in agent_statuses:
            # 跳过已停止/异常的 subagent (status值: running/waiting/done/stopped)
            if status_info and status_info[0] in ("stopped",):
                continue
