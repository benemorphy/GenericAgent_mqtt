    ...
    def search_entities(self, keyword: str) -> List[Dict[str, Any]]:
        ...
        rows = conn.execute(
            """SELECT e.name, e.description,
                      (SELECT GROUP_CONCAT(p2.key || '=' || p2.value, '; ')
                       FROM (SELECT DISTINCT p3.key, p3.value
                             FROM properties p3 WHERE p3.entity = e.name) p2
                      ) as props_sample
               FROM entities e
...
# ---- 写操作 API ----
    ...
    def add_entity(self, name: str, section: str = "EXPERIENCE", description: str = "") -> bool:
        """添加实体到知识图谱。如果已存在则返回 False。"""
        conn = self._get_conn()
        try:
            conn.execute(
                "INSERT OR IGNORE INTO entities (name, section, description) VALUES (?, ?, ?)",
                (name.upper(), section.upper(), description)
            )
            conn.commit()
    ...
    def add_property(self, entity: str, key: str, value: str) -> bool:
        """为实体添加属性。"""
        conn = self._get_conn()
        try:
            conn.execute(
                "INSERT INTO properties (entity, key, value) VALUES (?, ?, ?)",
                (entity.upper(), key, value)
            )
            conn.commit()
            return True
        except Exception:
    ...
    def add_relation(self, source: str, target: str, relation: str,
        ...
        try:
            existing = conn.execute(
                ...
                (source.upper(), target.upper(), relation.lower())
            ).fetchone()
            if existing:
                return False
            conn.execute(
                "INSERT INTO relations (source, target, relation, weight, description) VALUES (?, ?, ?, ?, ?)",
                (source.upper(), target.upper(), relation.lower(), weight, description)
            )
            conn.commit()
            return True
        except Exception:
