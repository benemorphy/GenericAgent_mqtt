    ...
    while turn < handler.max_turns:
        ...
        if _ROUTER_ENABLED and _ROUTER is not None:
            try:
                ...
                # ---------- P0/P1/P2 策略注入 ----------
                if decision.prompt_policy and decision.prompt_policy != "P0":
                    try:
                        from squilla_router.controller import get_prompt_hint
                        hint = get_prompt_hint(decision.prompt_policy, curr_text)
                        if hint and messages and messages[0].get('role') == 'system':
                            sys_content = messages[0].get('content', '')
                            if isinstance(sys_content, str):
                                tag = f"\n[Policy-{decision.prompt_policy}]: {hint}"
                                if tag not in sys_content:
                                    messages[0]['content'] = sys_content + tag
                                    if verbose:
                                        yield f"[Policy] {decision.prompt_policy} hint injected\n\n"
                    except Exception:
                        pass  # 策略注入失败不阻塞主流程
