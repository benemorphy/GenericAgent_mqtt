#!/usr/bin/env python3
"""skill_learn rev16 -- erlang_programming 验证工具模板
...
PATTERNS = [
  {
    "id": "P_domain_1",
    "principle": "erlang_programming核心概念与术语体系",
    "confidence": 75,
    "level": "domain"
  },
  {
    "id": "P_domain_2",
    "principle": "erlang_programming常见场景与解决方案",
    "confidence": 75,
    "level": "domain"
  },
  {
    "id": "P_domain_3",
    "principle": "erlang_programming工具链与环境搭建",
    "confidence": 75,
    "level": "domain"
  }
]
...
def _llm_generate_questions(patterns):
    ...
    _llm = _get_llm()
    prompt = f"""技能: erlang_programming
知识模式列表（JSON）：
{patterns_text}
...
def main():
    border = "=" * 55
    print(f"\n{border}")
    print(f"  rev16 验证 -- erlang_programming")
    print(f"{border}")
