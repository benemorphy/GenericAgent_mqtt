#!/usr/bin/env python3
"""skill_learn rev1 -- image_verification_satellite 验证工具模板
...
PATTERNS = [
  {
    "id": "P_domain_llm_1",
    "principle": "卫星图像几何校正：利用地理定位文件与地面控制点进行空间配准",
    "confidence": 92,
    "level": "domain"
  },
  {
    "id": "P_001",
    "principle": "在卫星图像分析中，必须使用地面真实数据（Ground Truth）进行验证，通过实地测量或已知参考点来校准和确认图像分析的准确性。",
    "confidence": 90,
    "level": "domain"
  },
  {
    "id": "P_003",
    "principle": "对原始卫星数据进行质量检查，识别并处理云覆盖、传感器故障等问题，确保数据源可靠后再进行后续处理。",
    "confidence": 88,
    "level": "domain"
  },
  {
    "id": "P_domain_llm_2",
    "principle": "卫星图像辐射校正与大气校正：消除传感器与大气干扰，恢复真实地表反射率",
    "confidence": 88,
    "level": "domain"
  },
  {
    "id": "P_002",
    "principle": "在完成卫星图像处理或分析任务前，必须执行验证命令并确认输出结果，避免在未验证的情况下做出完成或成功的声明。",
    "confidence": 85,
    "level": "domain"
  },
  {
    "id": "P_004",
    "principle": "进行坐标转换，使卫星数据适配客户的坐标系需求，确保地理空间参考的一致性。",
    "confidence": 85,
    "level": "domain"
  },
  {
    "id": "P_005",
    "principle": "将卫星数据转换为通用格式（如IMG或GEOTIFF），以保证与多种遥感GIS软件平台的兼容性。",
    "confidence": 85,
    "level": "domain"
  },
  {
    "id": "P_domain_llm_3",
    "principle": "卫星图像质量验证：评估分辨率、云覆盖、信噪比等指标以确保数据可用性",
    "confidence": 85,
    "level": "domain"
  },
  {
    "id": "P_006",
    "principle": "对于低分辨率卫星数据，由于地面控制点选择困难，应优先使用卫星自带的地理定位文件进行几何校正。",
    "confidence": 80,
    "level": "advanced"
  },
  {
    "id": "P_domain_llm_4",
    "principle": "AI生成卫星图像检测：识别深度伪造或合成图像，维护数据真实性",
    "confidence": 80,
    "level": "domain"
  },
  {
    "id": "P_domain_llm_5",
    "principle": "卫星图像数据集构建与标注：为深度学习模型训练提供高质量验证样本",
    "confidence": 78,
    "level": "domain"
  },
  {
    "id": "P_007",
    "principle": "利用深度学习模型（如CNN、ViT）检测AI生成的卫星图像（Deepfake Geography），并通过可解释性方法（如Grad-CAM、Chefer注意力归因）增强模型透明度和检测行为理解。",
    "confidence": 75,
    "level": "advanced"
  },
  {
    "id": "P_010",
    "principle": "利用深度学习进行卫星图像分割时，需针对卫星图像特征（如地物多样性）进行专门训练，以准确检测道路、建筑物等目标。",
    "confidence": 75,
    "level": "advanced"
  },
  {
    "id": "P_008",
    "principle": "构建高质量卫星图像数据集是训练和验证深度学习模型的基础，需确保数据集的版权合规性（如使用Apache License 2.0）。",
    "confidence": 70,
    "level": "domain"
  },
  {
    "id": "P_009",
    "principle": "在卫星图像变化检测等任务中，采用竞赛级解决方案（如1st place solution）中的深度学习技术（如分类、分割）来提升分析精度。",
    "confidence": 70,
    "level": "advanced"
  },
  {
    "id": "P_012",
    "principle": "参考遥感领域元分析研究（如ISPRS期刊综述），系统性地应用深度学习在遥感中的最佳实践，包括模型选择、训练策略和评估方法。",
    "confidence": 70,
    "level": "advanced"
  },
  {
    "id": "P_011",
    "principle": "在卫星图像验证中，通过分析建筑物阴影的方位角等几何特征，可以辅助判断图像的真实性和拍摄时间。",
    "confidence": 65,
    "level": "advanced"
  }
]
...
def _llm_generate_questions(patterns):
    ...
    _llm = _get_llm()
    prompt = f"""技能: image_verification_satellite
知识模式列表（JSON）：
{patterns_text}
...
def main():
    border = "=" * 55
    print(f"\n{border}")
    print(f"  rev1 验证 -- image_verification_satellite")
    print(f"{border}")
    ...
    result = {
        "version": 1,
        "skill": "image_verification_satellite",
