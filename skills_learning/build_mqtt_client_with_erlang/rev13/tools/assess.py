#!/usr/bin/env python3
"""skill_learn rev13 -- build_mqtt_client_with_erlang 验证工具模板
...
PATTERNS = [
  {
    "id": "P_domain_llm_1",
    "principle": "MQTT协议版本与特性实现（3.1/3.1.1/5.0）",
    "confidence": 95,
    "level": "domain"
  },
  {
    "id": "P_domain_llm_2",
    "principle": "Erlang客户端连接管理与重连机制",
    "confidence": 90,
    "level": "domain"
  },
  {
    "id": "P_001",
    "principle": "使用Erlang/OTP构建MQTT客户端时，应优先选择成熟的开源库（如emqtt、emqttc），这些库通常支持MQTT 3.1、3.1.1和5.0协议，并经过主流Broker（如Mosquitto、RabbitMQ、EMQX）的测试验证。",
    "confidence": 90,
    "level": "domain"
  },
  {
    "id": "P_domain_llm_3",
    "principle": "发布/订阅模式与消息QoS处理",
    "confidence": 88,
    "level": "domain"
  },
  {
    "id": "P_domain_llm_4",
    "principle": "客户端与MQTT Broker（如Mosquitto、EMQX）的集成测试",
    "confidence": 85,
    "level": "domain"
  },
  {
    "id": "P_002",
    "principle": "在Erlang shell中测试MQTT客户端时，需通过-pa参数添加编译后的库路径（如_build/default/lib/*/ebin），确保依赖模块可被加载。",
    "confidence": 85,
    "level": "domain"
  },
  {
    "id": "P_005",
    "principle": "Erlang MQTT客户端应支持TCP/SSL Socket连接，并实现自动重连机制，以增强在网络不稳定环境下的可靠性。",
    "confidence": 85,
    "level": "advanced"
  },
  {
    "id": "P_003",
    "principle": "MQTT客户端应支持通过命令行参数配置协议版本（-V）、用户名密码（-u/-P）、客户端ID（-C）、心跳间隔（-k）、主题（-t）和服务质量（-q），以提供灵活的连接选项。",
    "confidence": 80,
    "level": "domain"
  },
  {
    "id": "P_007",
    "principle": "MQTT客户端应支持QoS 0、QoS 1和QoS 2三种服务质量等级，以满足不同场景下消息传递的可靠性需求。",
    "confidence": 80,
    "level": "domain"
  },
  {
    "id": "P_domain_llm_5",
    "principle": "命令行工具开发与调试支持",
    "confidence": 78,
    "level": "domain"
  },
  {
    "id": "P_domain_1",
    "principle": "http相关技术与最佳实践（build_mqtt_client_wi）",
    "confidence": 77,
    "level": "domain"
  },
  {
    "id": "P_domain_2",
    "principle": "wiki相关技术与最佳实践（build_mqtt_client_wi）",
    "confidence": 77,
    "level": "domain"
  },
  {
    "id": "P_domain_3",
    "principle": "build_mqtt_client_with_erlang工具链与环境搭建",
    "confidence": 75,
    "level": "domain"
  },
  {
    "id": "P_004",
    "principle": "实现MQTT客户端时，应支持遗嘱消息（Will Message）功能，通过--will-topic和--will-payload参数配置，以便在客户端意外断开时通知其他订阅者。",
    "confidence": 75,
    "level": "domain"
  },
  {
    "id": "P_006",
    "principle": "在Erlang中开发MQTT客户端时，应遵循OTP设计原则，使用gen_server或gen_statem等行为模式管理连接状态，确保并发安全和状态一致性。",
    "confidence": 70,
    "level": "advanced"
  },
  {
    "id": "P_008",
    "principle": "对于Erlang MQTT客户端，建议使用Erlang R17+版本，以利用更完善的并发特性和库支持。",
    "confidence": 65,
    "level": "domain"
  }
]
...
def _llm_generate_questions(patterns):
    ...
    _llm = _get_llm()
    prompt = f"""技能: build_mqtt_client_with_erlang
知识模式列表（JSON）：
{patterns_text}
...
def main():
    border = "=" * 55
    print(f"\n{border}")
    print(f"  rev13 验证 -- build_mqtt_client_with_erlang")
    print(f"{border}")
    ...
    result = {
        "version": 13,
        "skill": "build_mqtt_client_with_erlang",
