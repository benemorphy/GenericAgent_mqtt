#!/usr/bin/env python3
"""skill_learn rev14 -- cypher_programming_language 验证工具模板
...
PATTERNS = [
  ...
  {
    "id": "P_002",
    "principle": "在MATCH子句中始终明确指定关系类型和方向，避免无关系条件的模式导致笛卡尔积，防止中间结果集爆炸。",
    "confidence": 95,
    "level": "domain"
  },
  {
  ...
  {
    "id": "P_001",
    "principle": "为频繁查询的节点属性创建索引，以加速节点查找和匹配操作。",
    "confidence": 90,
    "level": "domain"
  },
  {
  ...
  {
    "id": "P_003",
    "principle": "仅使用图数据库中已定义的节点类型、关系类型和属性，不引入未在模式中声明的元素。",
    "confidence": 85,
    "level": "domain"
  },
  {
    "id": "P_006",
    "principle": "在生成Cypher语句时，严格遵循提供的模式过滤规则，不使用未在模式中列出的关系类型或属性。",
    "confidence": 85,
    "level": "advanced"
  },
  {
  ...
  {
    "id": "P_004",
    "principle": "优化查询模式，减少不必要的UNWIND、OPTIONAL MATCH和聚合操作，以降低计算开销。",
    "confidence": 80,
    "level": "advanced"
  },
  {
  ...
  {
    "id": "P_005",
    "principle": "根据数据集大小和工作负载复杂度选择简单直接的查询模式，避免过度复杂的嵌套子查询。",
    "confidence": 75,
    "level": "domain"
  },
  {
    "id": "P_007",
    "principle": "使用参数化查询代替字符串拼接，以提高查询安全性和执行计划复用效率。",
    "confidence": 70,
    "level": "advanced"
  }
]
...
def main():
    border = "=" * 55
    print(f"\n{border}")
    print(f"  rev14 验证 -- cypher_programming_language")
    print(f"{border}")
