#!/usr/bin/env python3
"""skill_learn rev15 -- cypher_programming_language 验证工具模板
...
PATTERNS = [
  {
    "id": "P_domain_llm_1",
    "principle": "Cypher 查询基础与图模式匹配",
    "confidence": 95,
    "level": "domain"
  },
  {
    "id": "P_002",
    "principle": "在MATCH子句中明确指定关系模式，避免无关系条件的节点匹配导致笛卡尔积，防止中间结果集爆炸。",
    "confidence": 95,
    "level": "domain"
  },
  {
  ...
  {
    "id": "P_domain_llm_2",
    "principle": "数据建模与节点关系设计",
    "confidence": 90,
    "level": "domain"
  },
  {
    "id": "P_001",
    "principle": "为频繁查询的节点属性创建索引，以加速节点查找，避免全图扫描。",
    "confidence": 90,
    "level": "domain"
  },
  {
  ...
  {
    "id": "P_domain_llm_3",
    "principle": "高级查询：聚合、路径遍历与子查询",
    "confidence": 85,
    "level": "domain"
  },
  {
    "id": "P_003",
    "principle": "只返回查询所需的具体属性或数据，避免返回整个节点或关系，以减少数据传输和内存消耗。",
    "confidence": 85,
    "level": "domain"
  },
  {
  ...
  {
    "id": "P_domain_1",
    "principle": "Cypher查询语言核心语法与模式匹配（cypher_programming_l）",
    "confidence": 83,
    "level": "domain"
  },
  {
  ...
  {
    "id": "P_domain_llm_5",
    "principle": "Cypher 与 Neo4j 集成：事务管理与程序化查询",
    "confidence": 75,
    "level": "domain"
  },
  {
    "id": "P_004",
    "principle": "根据数据集大小和工作负载复杂度选择简单查询，避免过度复杂的模式匹配，以维持性能。",
    "confidence": 75,
    "level": "domain"
  },
  {
    "id": "P_005",
    "principle": "利用Cypher的模式匹配表达能力编写声明式查询，避免使用底层Core API或Traversal Framework，除非需要自定义剪枝等高级控制。",
    "confidence": 70,
    "level": "advanced"
  },
  {
    "id": "P_007",
    "principle": "当需要自定义路径遍历剪枝逻辑以提升性能时，考虑使用Traversal Framework替代Cypher。",
    "confidence": 65,
    "level": "advanced"
  },
  {
    "id": "P_006",
    "principle": "在查询优化中，使用性能-社会图模型分析节点和关系的连接依赖，以识别并消除低效模式。",
    "confidence": 60,
    "level": "advanced"
  }
]
...
def _llm_generate_questions(patterns):
    ...
    _llm = _get_llm()
    prompt = f"""技能: cypher_programming_language
知识模式列表（JSON）：
{patterns_text}
...
def main():
    border = "=" * 55
    print(f"\n{border}")
    print(f"  rev15 验证 -- cypher_programming_language")
    print(f"{border}")
    ...
    result = {
        "version": 15,
        "skill": "cypher_programming_language",
