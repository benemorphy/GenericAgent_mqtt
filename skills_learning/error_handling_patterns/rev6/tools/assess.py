#!/usr/bin/env python3
"""skill_learn rev6 -- error_handling_patterns 验证工具模板
...
PATTERNS = [
  ...
  {
    "id": "P_001",
    "principle": "在缺乏异常处理机制的语言中，使用返回码和errno模式来指示错误，而非依赖语言内置异常结构。",
    "confidence": 85,
    "level": "domain"
  },
  {
    "id": "P_003",
    "principle": "在统计假设检验中，区分第一类错误（假阳性，错误拒绝真零假设）和第二类错误（假阴性），以准确评估测试结果。",
    "confidence": 85,
    "level": "domain"
  },
  {
    "id": "P_005",
    "principle": "在发布-订阅模式中，通过解耦发布者和订阅者，避免同步模式如RPC和点对点消息的紧耦合，实现最高级别的松耦合。",
    "confidence": 85,
    "level": "domain"
  },
  {
  ...
  {
    "id": "P_002",
    "principle": "使用访问者模式时，通过编译器错误强制处理所有新定义的对象类型，确保类型安全。",
    "confidence": 80,
    "level": "domain"
  },
  {
    "id": "P_004",
    "principle": "异常处理语法应提供关键字和结构，将错误处理代码与正常逻辑分离，提高代码可读性和维护性。",
    "confidence": 80,
    "level": "domain"
  },
  {
...
def main():
    border = "=" * 55
    print(f"\n{border}")
    print(f"  rev6 验证 -- error_handling_patterns")
    print(f"{border}")
