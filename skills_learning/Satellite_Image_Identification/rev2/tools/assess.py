#!/usr/bin/env python3
"""skill_learn rev2 -- satellite_image_identification 验证工具模板
...
PATTERNS = [
  ...
  {
    "id": "P_domain_llm_1",
    "principle": "Satellite Image Preprocessing (几何校正、辐射定标与大气校正)",
    "confidence": 95,
    "level": "domain"
  },
  {
  ...
  {
    "id": "P_domain_llm_2",
    "principle": "Multispectral and Hyperspectral Image Analysis (多光谱与高光谱影像分析)",
    "confidence": 92,
    "level": "domain"
  },
  {
  ...
  {
    "id": "P_001",
    "principle": "利用多时相卫星影像进行变化检测，通过比较同一区域不同时间点的图像来识别动态现象，如臭氧空洞的演变。",
    "confidence": 90,
    "level": "domain"
  },
  {
    "id": "P_005",
    "principle": "构建卫星影像时间序列数据集，通过分析时间维度的光谱和纹理变化，监测地表动态过程。",
    "confidence": 90,
    "level": "advanced"
  },
  {
    "id": "P_domain_llm_3",
    "principle": "Land Cover and Land Use Classification (土地覆盖与土地利用分类)",
    "confidence": 90,
    "level": "domain"
  },
  {
  ...
  {
    "id": "P_domain_llm_4",
    "principle": "Change Detection and Time Series Analysis (变化检测与时间序列分析)",
    "confidence": 88,
    "level": "domain"
  },
  {
    "id": "P_002",
    "principle": "在卫星图像解译中，结合上下文和已知地物特征进行视觉分析，以识别和分类图像中的目标。",
    "confidence": 85,
    "level": "domain"
  },
  {
    "id": "P_004",
    "principle": "利用多光谱成像技术，通过多个离散波段的数据融合，增强对军事目标、植被或地质特征的识别能力。",
    "confidence": 85,
    "level": "domain"
  },
  {
    "id": "P_domain_llm_5",
    "principle": "Object-Based Image Analysis (面向对象影像分析)",
    "confidence": 85,
    "level": "domain"
  },
  {
  ...
  {
    "id": "P_003",
    "principle": "采用高光谱成像技术，通过分析连续窄波段的光谱信息，实现化学物质和材料的精确检测与识别。",
    "confidence": 80,
    "level": "advanced"
  },
  {
...
def _llm_generate_questions(patterns):
    ...
    _llm = _get_llm()
    prompt = f"""技能: satellite_image_identification
知识模式列表（JSON）：
{patterns_text}
...
def main():
    border = "=" * 55
    print(f"\n{border}")
    print(f"  rev2 验证 -- satellite_image_identification")
    print(f"{border}")
    ...
    result = {
        "version": 2,
        "skill": "satellite_image_identification",
