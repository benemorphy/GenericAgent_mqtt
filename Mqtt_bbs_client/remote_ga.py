...
环境变量:
  ...
  BROKER_USER  MQTT 用户名 (默认 remote_ga)
  BROKER_PASS  MQTT 密码 (必需)
  BOARD_NAME   Board 名称 (默认 remote_ga)
  BOARD_KEY    Board 标识键 (默认 inspiration)
...
def main():
    ...
    broker_user = os.getenv("BROKER_USER", "remote_ga")
    broker_pass = os.environ.get("BROKER_PASS")
    board_name = os.getenv("BOARD_NAME", "remote_ga")
    board_key = os.getenv("BOARD_KEY", "inspiration")
