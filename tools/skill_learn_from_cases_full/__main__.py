...
from tools.skill_learn_from_cases_full.engine import learn_skill
from tools.skill_learn_from_cases_full.dir_manager import get_all_skills
...
"""
    ...
    parser.add_argument(
        "--from-url",
        type=str,
        help="从指定URL提取内容作为学习案例，如 --from-url https://... "
    )
    ...
    if args.dry_run:
        ...
        # 环境探测
        try:
            from tools.skill_learn_from_cases_full.env_detector import detect_all
            env = detect_all()
    ...
    learn_skill(en_name, from_url=args.from_url)
