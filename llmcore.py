...
from tools.history.compression_pipeline import CompressionPipeline
...
_pipeline = CompressionPipeline()  # 新管线，按需启用
...
def trim_messages_history(history, sess):
    ...
    # 新管线优先（动态触发+层级摘要+语义裁剪+压缩标记），失败时回退旧压缩器
    try:
        _pipeline.compress(history, sess=sess, turn=getattr(sess, 'turn_count', 0))
    except Exception:
    ...
    if cost() <= cap: return
    try:
        _pipeline.compress(history, turn=getattr(sess, 'turn_count', 0))
    except Exception:
...
class BaseSession:
    def __init__(self, cfg):
        ...
        self.connect_timeout = max(1, int(cfg.get('connect_timeout', default_ct)))
